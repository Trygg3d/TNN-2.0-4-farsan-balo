...
class DynamicTimeAttention(nn.Module):
    def __init__(self, feature_dim, num_heads):
        super(DynamicTimeAttention, self).__init__()
    ...
    def forward(self, query, key, value, mask=None):
        ...
        adaptive_weights = self.adaptive_weight_net(query).unsqueeze(-1)
        ...
        # Broadcasting adaptive_weights across the seq_length dimension for scaling
        scaled_attention_scores = attention_scores * adaptive_weights

        attention_output = torch.matmul(scaled_attention_scores, value)

        attention_output = attention_output.transpose(1, 2).contiguous().view(batch_size, seq_length, self.feature_dim)
...
class TransformerBlock(nn.Module):
    def __init__(self, input_dim, num_heads, dropout_rate, forward_expansion):
        ...
        self.attention = DynamicTimeAttention(input_dim, num_heads)
...
# Example: Create sequences of 2 days
sequence_length = 2
